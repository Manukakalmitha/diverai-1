    ...
    return (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900/90 backdrop-blur-xl border-t border-slate-800 z-50 pb-safe">
            <div className="flex justify-around items-center h-16 px-2">
                ...
                <Link to="/pricing" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${isActive('/pricing') ? 'text-emerald-400' : 'text-slate-500'}`}>
                    <Crown className={`w-5 h-5 ${isActive('/pricing') ? 'fill-emerald-500/20' : ''}`} />
                    <span className="text-[9px] font-black uppercase tracking-widest">Pricing</span>
                </Link>
                <Link to="/docs" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${isActive('/docs') ? 'text-emerald-400' : 'text-slate-500'}`}>
                    <Book className={`w-5 h-5 ${isActive('/docs') ? 'fill-emerald-500/20' : ''}`} />
